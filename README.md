## MyMatrixC
Реализация библиотеки my_matrix.h.
В данном проекте реализована собственная библиотека для обработки числовых матриц на языке программирования Си. Матрицы являются одной из базовых структур данных в программировании, например, они применяются для представления табличных значений, для вычислительных задач и нейронных сетей.
### Матрица
...
### Создание матриц (my_create_matrix)

```c
int my_create_matrix(int rows, int columns, matrix_t *result);
```

### Очистка матриц (my_remove_matrix)

```c
void my_remove_matrix(matrix_t *A);
```

### Сравнение матриц (my_eq_matrix)
...
### Сложение (my_sum_matrix) и вычитание матриц (my_sub_matrix)
...
### Умножение матрицы на число (my_mult_number). Умножение двух матриц (my_mult_matrix)
...
### Транспонирование матрицы (my_transpose)
...
### Минор матрицы и матрица алгебраических дополнений (my_calc_complements)
...
### Определитель матрицы (my_determinant)
...
### Обратная матрица (my_inverse_matrix)
...
### Реализация функций библиотеки my_matrix.h

- Библиотека разработана на языке Си стандарта C11 с использованием компилятора gcc 
- Код соответствует Google Style
- Решение оформлено как статическая библиотека (с заголовочным файлом my_matrix.h)
